...
func prepare(
	...
	newID IDFunc,
	now time.Time,
	crawler *fingerprint.CrawlerVerifier,
	...
	if ua := r.Header.Get("User-Agent"); ua != "" {
		...
		// impersonator; a transient DNS failure abstains (never convicts a real crawler). Only the rare declared-
		// crawler UA pays the cost (the CIDR check is in-memory; only the FCrDNS fallback can do I/O).
		if crawler != nil && fingerprint.DeclaredCrawler(ua) != nil {
			if ip := clientIP(r); ip != "" {
				ctx, cancel := context.WithTimeout(r.Context(), 800*time.Millisecond)
				if crawler.Verify(ctx, ua, ip) == fingerprint.CrawlerFake {
	...
	// A non-Chromium User-Agent (Safari or Firefox) that ALSO sends Sec-CH-UA — a network-layer contradiction.
	// User-Agent Client Hints are Blink-ONLY: Safari (macOS + iOS) never shipped them and Firefox declined them
	// (Mozilla, privacy). So a UA claiming Safari/Firefox while any Sec-CH-UA header is present is a Chromium
	// engine wearing that UA. Caught at the HEADER layer, so it fires even for a NO-JS scraper the JS-based
	// br.apple_ua_nonwebkit / firefox_ua_nongecko cannot see. FP-safe: no real Safari or Firefox emits Sec-CH-UA.
...
func NewReverseProxy(backendURL, detectorURL string, hints fingerprint.HintTable) (*ReverseProxy, error) {
	...
	return &ReverseProxy{
		...
		crawler: &fingerprint.CrawlerVerifier{
			...
			// Deploy-refreshed Google/Bing CIDR feeds (DNS-free); empty when KITSUNE_CRAWLER_CIDR_DIR is unset
			// or unrefreshed → verification falls back to FCrDNS (the prior behaviour), never a stale conviction.
			CIDR: fingerprint.LoadCrawlerCIDR(os.Getenv("KITSUNE_CRAWLER_CIDR_DIR")),
		},
...
func (p *ReverseProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if p.synStore != nil {
		if k, mss, wscale, wscalePresent, ok := p.synStore.Get(clientIP(r)); ok {
			if k != "" {
				prep.signals = append(prep.signals, signal.Network(prep.sessionID, "tcp_kernel", k, p.now()))
			}
			// Raw TCP MSS from the SYN — the tunnel/proxy-egress tell (a lowered MSS betrays an encapsulating
			// VPN/SOCKS hop). A clean-room network-layer field, not the FoxIO JA4T composite it replaces.
			if mss != 0 {
				prep.signals = append(prep.signals, signal.Network(prep.sessionID, "tcp_mss", int(mss), p.now()))
			}
			// SYN value-vs-order coherence (p0f-grounded): the option ORDER classifies the kernel, but the
			// VALUES must match that OS too — a darwin-ordered SYN with window scale > 5 (no real macOS/iOS
			// emits that) is an order-copying forge. Reads the window scale straight from the parsed SYN.
			if fingerprint.SYNValueAnomaly(k, int(wscale), wscalePresent) {
