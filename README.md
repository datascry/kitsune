...
**134 live rules** (102 active · 32 experimental; 6 retired, ruleset `0.74.51`) — each a small predicate over the correlated session. **92 can convict** (coherence/automation/artifact); the rest only corroborate. Grouped by detection class:

| Class | Rules | Convicts? | What it catches |
|---|---:|:--:|---|
| **coherence** | 53 | ✦ | cross-vector contradictions (TLS↔TCP↔UA↔JS↔h2↔QUIC) — the thesis core |
...
**88 of 98 evaders score `bot`** ([full matrix](docs/matrix.md), ruleset `0.74.51`). The remaining 10 reach only `suspicious` — the conviction-gate frontier (top evaders, below): they defeat every *convicting* rule and trip only corroborating tells, which can never reach `bot` alone.
