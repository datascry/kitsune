...
**141 live rules** (106 active · 35 experimental; 6 retired, ruleset `0.74.55`) — each a small predicate over the correlated session. **97 can convict** (coherence/automation/artifact); the rest only corroborate. Grouped by detection class:
...
| **behavioral** | 13 | — | mouse/keystroke biomechanics — path straightness, velocity CV, entropy floors |
...
**93 of 103 evaders score `bot`** ([full matrix](docs/matrix.md), ruleset `0.74.55`). The remaining 10 reach only `suspicious` — the conviction-gate frontier (top evaders, below): they defeat every *convicting* rule and trip only corroborating tells, which can never reach `bot` alone.
