...
**192 live rules** (133 active · 59 experimental; 6 retired, ruleset `0.74.57`) — each a small predicate over the correlated session. **145 can convict** (coherence/automation/artifact); the rest only corroborate. Grouped by detection class:

| Class | Rules | Convicts? | What it catches |
|---|---:|:--:|---|
| **coherence** | 87 | ✦ | cross-vector contradictions (TLS↔TCP↔UA↔JS↔h2↔QUIC) — the thesis core |
| **automation** | 42 | ✦ | the framework surface: `webdriver`, CDP runtime, Electron, isolated-world leaks |
