...
| `CANVAS_LIE=1` | canvas-lie | overrides `HTMLCanvasElement.prototype.toDataURL` with a plain (non-native) wrapper — its `toString` lacks `[native code]` → `br.canvas_lie` (automation). The naive canvas-fingerprint blocker. A real `toDataURL` is native. Gives that active rule its first live positive. |
| `DOMRECT_SPOOF=1` | domrect-spoof | adds per-call sub-pixel noise to `Element.getBoundingClientRect`, so two reads of an unchanged element differ — breaking the determinism invariant → `br.domrect_invariant` (artifact). A real engine's `getBoundingClientRect` is deterministic. Gives that active rule its first live positive. |
