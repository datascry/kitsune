...
func main() {
	...
	s := azuretls.NewSession(); s.InsecureSkipVerify = true //nolint:gosec // lab edge self-signed cert (allow-listed)
	...
	cl := &http.Client{Transport:&http.Transport{TLSClientConfig:&tls.Config{InsecureSkipVerify:true}}} //nolint:gosec // lab detector self-signed cert (allow-listed)
