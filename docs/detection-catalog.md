...
> Every detection rule Kitsune leverages — **generated** from `contracts/rules/registry.yaml` (ruleset `0.74.32`); regenerate with `task catalog`, do not edit by hand. **122 rules**: 89 active · 29 experimental · 4 retired; 82 convicting (coherence/automation/artifact — only these can convict a `bot`; environment/behavioral/reputation/prevalence corroborate only).
