...
|---|---|---|---|
| **Chrome `--headless=new` (the modern token-less headless) + a CDP-minimal driver** | high | `br.headless_ua`, `br.ch_he_headless` — and on a CDP-minimal driver (nodriver), its ONLY convicting tell | GROUNDED 2026-06-20: nodriver is caught by a SINGLE convicting tell, `br.headless_ua` (the old `--headless` "HeadlessChrome" UA token). Chrome's `--headless=new` carries a NORMAL UA + Sec-CH-UA (no HeadlessChrome), so both headless-reveal tells go silent → a stock-Chrome new-headless tool (nodriver/undetected `--headless=new`) joins the EVADES frontier (camoufox-headful/zendriver/patchright-headful). **No FP-safe per-session CONVICTING replacement**: new-headless's traits (software WebGL / no media devices / no display) are ENVIRONMENTAL — real GPU-less machines (VMs/servers/GPU-disabled Chrome) share them, so they corroborate, never convict. Answerable only by the structural frontier (coordination). NB: the Playwright stealth evader CANNOT test this — its bundled Chromium reports HeadlessChrome even with `--headless=new` (not a stock build). |
