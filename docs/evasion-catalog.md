...
| **JA4H regular-header-order forgery (curl-impersonate / primp / go-tls)** | confirmed-defeated (re-validated live 2026-06-19) | net.h2_header_order_vs_ua (experimental), net.h2_engine_unknown | Live iter-8 fleet run, **re-confirmed on the live detector+edge stack (2026-06-19)**: every chromium-claiming HTTP-impersonation stack already replicates Chrome's *regular* header order (sec-ch-ua before user-agent), so the edge never emits `h2_header_order_non_chromium` and the rule yields zero positives; the only non-Chromium stack in the fleet (httpx/vanilla) speaks HTTP/1.1, so it has no h2 order to mismatch. The read signal is genuinely never produced by the current fleet — header-order detection needs a non-Chromium h2 stack, or a co-signal (settings-vs-order disagreement), to bite. _(Also surfaced: the locally-built `kitsune-detector` image is stale at ruleset v0.71.0 — a live scoreboard run should rebuild it; the committed matrix is already regenerated against the current ruleset via `task matrix`.)_ |
