...
| `arena-solver-ocr` | Python | OCR evader that beats the arena's distorted-text gate (owned gates only). |
...
### Techniques exercised — scored against the live ruleset (105)
...
| `apify-fp-inject` | bot | `br.cdp_runtime_enabled`, `br.languages_worker_vs_main`, `br.no_chrome_object`, `br.permissions_anomaly`, `br.webgl_worker_vs_main`, `br.worker_divergence` |
...
| `brave` | bot | `br.headless_ua`, `br.webdriver_present` |
...
| `chromium-firefox-ua-spoof` | bot | `br.battery_api_vs_gecko_ua`, `net.ch_ua_vs_ua_browser`, `net.h2_vs_js_ua_browser`, `net.h2_vs_ua_browser`, `net.tls_vs_js_ua_browser`, `net.tls_vs_ua_browser` |
...
| `datacenter-origin-proxied` | bot | `net.datacenter_origin_proxied` |
...
| `firefox-coherent` | bot | `br.webdriver_present` |
...
| `go-tls-madeyoureset` | bot | `net.accept_encoding_vs_ua`, `net.h2_header_order_vs_ua`, `net.h2_madeyoureset`, `net.h2_unknown_vs_ua`, `net.no_js_execution`, `net.sec_fetch_vs_ua` |
...
| `playwright-extra-coherent` | bot | `br.cdp_runtime_enabled`, `br.ch_he_headless`, `br.languages_worker_vs_main`, `br.webgl_worker_vs_main`, `br.worker_divergence` |
...
| `trace-replay` | bot | `bh.trace_replay_within_session`, `br.cdp_runtime_enabled`, `br.ch_he_headless`, `br.headless_ua`, `br.no_chrome_object`, `br.permissions_anomaly`, `br.webdriver_spoofed` |
...
| `ua-rotation` | bot | `net.accept_encoding_vs_ua`, `net.h2_header_order_vs_ua`, `net.h2_unknown_vs_ua`, `net.no_js_execution`, `net.sec_fetch_vs_ua`, `net.tcp_os_vs_ua`, `net.tls_grease_vs_ua`, `net.ua_rotation_within_session` |
...
| `webgl-caps-worker-spoof` | bot | `br.automation_globals`, `br.cdp_runtime_enabled`, `br.webdriver_present`, `br.webgl_caps_worker_vs_main`, `br.webgl_getparameter_tampered`, `net.tls_ext_order_static_within_session` |
| `webgl-renderer-spoof` | bot | `br.automation_globals`, `br.cdp_runtime_enabled`, `br.ch_he_headless`, `br.headless_ua`, `br.no_chrome_object`, `br.permissions_anomaly`, `br.webdriver_present`, `br.webgl_getparameter_tampered`, `br.webgl_os_vs_ua`, `br.webgl_renderer_caps_mismatch`, `br.webgl_worker_vs_main`, `net.tls_ext_order_static_within_session` |
...
| `webkit-ua-spoof` | bot | `br.error_engine_vs_ua`, `br.navplatform_vs_ua`, `br.no_chrome_object`, `br.vendor_vs_ua`, `br.webdriver_present`, `br.webgl_os_vs_ua`, `net.h2_header_order_vs_ua`, `net.h2_unknown_vs_ua`, `net.tcp_os_vs_ua`, `net.tls_grease_vs_ua`, `net.tls_vs_js_ua_browser`, `net.tls_vs_ua_browser` |
...
| **fingerprint-injector (apify, v2.1.82)** — ⛔ defeats the artifact layer | White-boxed `utils.js` (2026-06-19): it overrides every property with `new Proxy(<the ORIGINAL native fn>, handler)` (`overrideGetterWithProxy`), NOT a plain function — so the proxy inherits the real native's invariants: no own `prototype`, non-constructable. Our `br.native_invariant_violated` (demo.py:537-540) checks exactly those, so by construction it is DEFEATED. Its `redirectToString` wraps `Function.prototype.toString` in a Proxy that (a) returns `makeNativeString` for its proxied objects and toString-of-toString, and (b) forwards other calls to the original `target.call(ctx)` — which still THROWS on a non-function `this`. So `br.function_tostring_tampered` (the "[native code]" check) AND a candidate `Function.prototype.toString.call({})`-must-throw check are BOTH defeated. **Conclusion (no rule shipped):** a sophisticated Proxy-over-native injector defeats the whole per-session artifact + coherence layer by design; the injected VALUES are real-traffic-sampled (probable joints), so they also clear hard-coherence rules and may clear the prevalence model. This is the concrete proof that the durable signal is **coordination (bots-at-scale)** + the **prevalence/likelihood** model on genuinely-improbable joints — not more per-session artifact tells. A naive (non-Proxy) toString patch would still be caught by `function_tostring_tampered`; the toString-throw check adds nothing over that and needs a contrived evader, so it was NOT shipped (constraint #6). **⚠️ GROUNDED LIVE iter-14 (2026-06-20) — the white-box "defeats the per-session artifact + coherence layer by design" conclusion was INCOMPLETE.** Built the `apify-fp-inject` evader (18th fleet tool: fingerprint-generator samples a real Linux-Chrome joint, `newInjectedContext` injects it) and ran it against the live stack. It DOES defeat the artifact layer (no `native_invariant`/`tostring_tampered`) AND the network layer (`network` score **0** — coherent Sec-CH-UA + headers, no `headless_ua`, no `ch_he_headless`). But the injector wires the fingerprint into the **MAIN realm only** (an init script), so a Web Worker still sees the un-injected headless navigator → it trips **THREE realm-coherence rules the un-spoofed browser never had**: `br.worker_divergence` (0.8), `br.languages_worker_vs_main` (0.6), `br.webgl_worker_vs_main` (0.6) — plus the residual automation floor (`cdp_runtime_enabled`, `permissions_anomaly`, `no_chrome_object`). Scored `bot` (0.99999). **The injection does not reduce detectability — it MANUFACTURES a new coherence contradiction** (a textbook live demonstration of the core thesis). No new rule needed (existing realm-coherence catches it); frozen `corpus/sessions/apify-fp-inject.json` + lit-test guard on `br.worker_divergence`. To beat the realm layer the injector would have to spoof the worker/OffscreenCanvas realm identically — which apify does not do — so worker-realm coherence is a robust FP-safe catch for the entire main-realm-injection class. Lesson (4th time): a white-box "defeats by design" inference is unreliable until grounded; live grounding surfaced the real catch the static read missed. |
