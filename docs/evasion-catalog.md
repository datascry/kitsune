...
|---|---|---|---|
| **Chrome `--headless=new` (the modern token-less headless) + a CDP-minimal driver** | high | `br.headless_ua`, `br.ch_he_headless` — the ONLY convicting tell on the ENTIRE CDP-minimal anti-detect class | GROUNDED 2026-06-20 (fragile-catch audit over the committed fleet): **FOUR real anti-detect tools — `nodriver`, `pydoll`, `selenium-driverless`, `undetected` — are EACH caught by a SINGLE convicting tell, `br.headless_ua`** (the old `--headless` "HeadlessChrome" UA token). They hide webdriver + minimise CDP (so `br.webdriver_present`/`br.cdp_runtime_enabled` stay silent) and hide it NATIVELY (no `br.webdriver_getter_tampered`), leaving the old-headless UA token as their lone convicting tell. Chrome's `--headless=new` carries a NORMAL UA + Sec-CH-UA (no HeadlessChrome), so it goes silent → the WHOLE class joins the EVADES frontier (camoufox-headful/zendriver/patchright-headful), not just one tool. **No FP-safe per-session CONVICTING replacement**: new-headless's traits (software WebGL / no media devices / no display) are ENVIRONMENTAL — real GPU-less machines (VMs/servers/GPU-disabled Chrome) share them, so they corroborate, never convict. Answerable only by the structural frontier (coordination). NB: the Playwright stealth evader CANNOT test this — its bundled Chromium reports HeadlessChrome even with `--headless=new` (not a stock build); use a stock-Chrome driver. |
