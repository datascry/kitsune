...
| `pursuit` | **Follow the dot** · smooth-pursuit tracking | keep the cursor on a continuously moving dot for a few seconds; a human's eye-hand pursuit trails the target with **tens of px** of error (visuomotor lag + jitter), while a bot that computes the deterministic path holds within a few px — superhuman tracking accuracy convicts (`bh.arena_pursuit_superhuman`) — a distinct **continuous-tracking** tell | a human (trails with tens of px error) | duration |
| `count` | **Count the circles** · counting | "how many &lt;colour&gt; circles?" — a bot CV-counts the shapes instantly while a human scans each one; a correct answer faster than a human can scan the whole scene convicts (`bh.arena_count_superhuman`) — reuses the solve-speed tell | a human (counts over a few seconds) | shape count |
