...
| G16 | network (DDoS L7) | **Slow-HTTP attacks** (slowloris / slow-POST / slow-read) — exhaust the connection table by HOLDING connections with partial requests / tiny windows. A DIFFERENT mechanism from the frame-floods Kitsune already catches; the edge's H2FrameScanner won't see it (it needs connection-duration/incompleteness accounting). | slowhttptest, Torshammer, slowloris | DDoS deep-dive 2026; see docs/adversary-emulation-roadmap.md | **lead (groundable)** → edge slow/incomplete-connection detector → `net.slow_http_attack`. Ground with a slowhttptest-style evader mode against the local edge. The clearest L7-DoS gap. |
