...
| G32 | standards | **Web Bot Auth nonce-replay** — RFC 9421 nonce must be unique in the validity window; an in-window replay verifies cleanly (reaches the verified allow-list) — the expiry arm of G25 structurally misses it. Per-keyid seen-nonce set → `net.web_bot_auth_nonce_replay` (convicts; withholds `web_bot_auth_verified`). | credential-capture/replay of a verified agent's signed request; go-tls KS_WEBBOTAUTH in-window same-nonce mode | draft-meunier-web-bot-auth-architecture-05 §4.2.2; Cloudflare secure-agentic-commerce | **lead (groundable, high).** Extends shipped G25; cryptographic proof says "fresh signer", repeated nonce proves "captured-credential replay". |
