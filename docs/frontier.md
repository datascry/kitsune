...
| wild-captcha styles / challenge-interaction | **earned-closed** (blue) | every prevalent wild captcha STYLE is now reproduced as an owned arena gate (31 gates) and every distinct **server-observed solve-behaviour tell family** is shipped + live-grounded both directions: solve-speed floor, temporal-precision-std (timing/cadence), click/point precision (locate/spotdiff), spatial-jitter/fidelity absence (hold-tremor/stroke), plan-length/optimality (slide), reaction-latency floor (reaction), continuous-tracking accuracy (pursuit), and structural (keymap/shell/track). 16 vendor wire-replicas compose over the same coherence verdict. The principle holds throughout: a solved gate is a Turing/cost test, the SERVER-OBSERVED solve-behaviour is what discriminates | remaining styles reuse an existing tell family (counting/maze/drag-sort = solve-speed); new tell families are external/infra-bound (force-pressure + multi-touch = touch hardware; working-memory-span = not human-passable). The frontier returns to fingerprint/coordination coherence |
...
- **`KS_GL_RENDERER`/`KS_GL_VENDOR`** (stealth **Chromium**) — a **patched Mesa llvmpipe** (`evaders/stealth/mesa-patch/build.sh`) reports a hardware GPU renderer/vendor **natively** (no JS spoof), so Chromium's WebGL renderer is coherent: `webgl_software` + caps + tamper + worker all silent (grounded 2026-07-06). The engine-level renderer fix ADR-0008 first called external-bound — now in-sandbox via a ~3-line driver patch. **Chromium-only** (grounded 2026-07-07): WebKit self-normalises its WebGL renderer to Safari's fixed `Apple GPU`/`WebKit` and ignores the GL driver, so the patch is moot for it — but WebKit never leaked the software renderer either (its GPU is coherent by its own normalisation). Residual `webgpu_webgl_vs` is env-bound (Dawn+software-Vulkan hangs headful Chromium; lavapipe proven to report the GPU at the Vulkan level)
- **The composition is per-layer, not one tool** — see [`coherent-stack.md`](coherent-stack.md); the GPU-caps "hardware wall" **fell in software**, and the Chromium **renderer wall fell too** (patched Mesa; WebKit's was never leaking — it self-normalises), so a **Linux-target morph is fully coherent in-sandbox on Firefox AND coherent-except-`webgpu` on Chromium**; what's left is cross-OS native TLS (target-OS browser), camoufox's mac DB caps bug, the mobile core count, and the env-bound WebGPU adapter
