...
> **Two browser-side collectors — know which is which (they are NOT interchangeable).** "The collector"
> is really **two** implementations of the in-page probe suite, with distinct jobs and signal coverage:
...
>   *full* probe suite, served at `/` (the public inspector at kitsune.id) and exercised by the live stack +
>   every evader capture, POSTing to `/ingest`. **This is the authoritative full collector that rules are
>   developed and validated against**, and the page a real visitor lands on — an edge-backed verdict with the
>   coherence spine, the composite fingerprint ID, and the network/reputation layers the edge actually reads.
>   (So "the detector mirrors the collector" in §10 means demo.py reproduces the production-collector signal
>   shape, not that the detector itself collects in production.)
>
> A former third collector — a standalone client-only self-test page (`collector/src/livepage/`, CreepJS /
> sannysoft-style, scored in-browser with no POST) — was **removed**: it duplicated `demo.py`'s job, was never
> deployed, and diverged in look. `demo.py` is the one canonical inspector. For white-box probing, drive the
> live stack / `demo.py`, not a separate page.
