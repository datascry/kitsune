...
|---|---|---|---|
| **Real-traffic prevalence** (X4) | a few hundred+ real-user sessions | hosted-demo opt-in export, or `corpus/sessions/` from real visitors | `task grounding -- <dir> --build-prior <prior.json>` then `task prevalence-real-corroborate <csv>` | promote `br.fingerprint_improbable` convicting (detection power vs same-distribution injectors) |
