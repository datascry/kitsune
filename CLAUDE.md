...
| `collector/` | TypeScript | In-browser signal collection — a **focused production** page script (`src/index.ts`→`collect.ts`). NB: the detector serves its OWN **full inline collector** (`detector/…/demo.py`) — the authoritative full suite rules are validated against, and the public inspector at kitsune.id. Two collectors, distinct jobs — see [`docs/architecture.md`](docs/architecture.md) §3. |
