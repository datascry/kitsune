...
def test_arena_all_relays_forward_when_configured(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert client.get("/arena/presshold", params={"level": "easy"}).status_code in ok
    assert client.post("/arena/presshold/verify", json={"id": "x", "held_ms": 2000, "samples": []}).status_code in ok
...
def test_arena_relay_200_forwards_and_anomaly_join(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # the presshold relay's 200-branch: GET + the /verify join (hold_robotic -> arena_hold_robotic)
    monkeypatch.setattr("kitsune_detector.app.httpx.AsyncClient", _bench_async_client({"anomaly": "hold_robotic"}))
    assert client.get("/arena/presshold", params={"level": "easy"}).status_code == 200
    assert (
        client.post(
    ...
    # the locate relay's 200-branch: GET + the /verify join (localize_superhuman -> arena_localize_superhuman)
    monkeypatch.setattr(
        "kitsune_detector.app.httpx.AsyncClient", _bench_async_client({"anomaly": "localize_superhuman"})
    )
    assert client.get("/arena/locate", params={"level": "easy"}).status_code == 200
    assert (
        client.post(
            "/arena/locate/verify", json={"id": "x", "x": 10, "y": 10}, headers={"Cookie": "ks_sid=s"}
        ).status_code
        == 200
    )
    # the match relay's 200-branch: GET + the /verify join (match_superhuman -> arena_match_superhuman)
    monkeypatch.setattr("kitsune_detector.app.httpx.AsyncClient", _bench_async_client({"anomaly": "match_superhuman"}))
    ...
    # the pattern relay's 200-branch: GET + the /verify join (pattern_superhuman -> arena_pattern_superhuman)
    monkeypatch.setattr(
        "kitsune_detector.app.httpx.AsyncClient", _bench_async_client({"anomaly": "pattern_superhuman"})
    )
    assert client.get("/arena/pattern", params={"level": "easy"}).status_code == 200
    assert (
        client.post(
            "/arena/pattern/verify",
            json={"id": "x", "stroke": [[0, 0], [1, 1]]},
            headers={"Cookie": "ks_sid=s"},
        ).status_code
        == 200
    )
    ...
    # the count relay's 200-branch: GET + the /verify join (count_superhuman -> arena_count_superhuman)
    monkeypatch.setattr("kitsune_detector.app.httpx.AsyncClient", _bench_async_client({"anomaly": "count_superhuman"}))
    assert client.get("/arena/count", params={"level": "easy"}).status_code == 200
    assert (
        client.post("/arena/count/verify", json={"id": "x", "guess": 3}, headers={"Cookie": "ks_sid=s"}).status_code
        == 200
    )
