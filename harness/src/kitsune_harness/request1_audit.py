...
# "no-JS" but NOT part of the pre-JS coherence verdict: excluded from the tally, reported separately.
...
#
# Framing note: "request-1" here means the edge's PRE-JS coherence verdict — the tell fires from network
# signals before the collector ever posts, not literally on the first TCP segment. Most fire on request 1;
# net.ch_ua_mobile_no_model reads the high-entropy Sec-CH-UA-Model, which arrives on request 2 after the
# edge's Accept-CH — still well before any JS runs. That is the botwall-relevant window (allow/challenge/deny
# before the page executes), so those tells are kept in the set; the grace/multi-connection ones above are not.
...
def render(report: Request1Report) -> str:
    ...
    lines.append(
        ...
        "edge concludes **before any JavaScript runs**. Every capture is stripped to its network + "
        'reputation layers and scored with the current ruleset. "Request-1" is the pre-JS window: most '
        "tells fire on the first request, a couple of Client-Hint tells arrive on request 2 after the edge's "
        "`Accept-CH` — both well before the collector posts.\n"
    )
    lines.append(
